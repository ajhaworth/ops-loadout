...
- **`setup.py` is the config; the `.blend` files are its output.**
  `portable/config/startup.blend` and `userpref.blend` are generated by
  `setup.py` on every install/Update and are gitignored, not committed - a
  hand-saved `.blend` would bake in per-machine absolute paths, lose the
  comments justifying each setting, and miss new Blender defaults. To change
  scene/UI/prefs, edit `setup.py` and re-run `bin/blender --python setup.py`
  (windowed, not `-b` — it needs a live window to walk the workspaces).
